...
if1 = f1_score(pred, test_labels)
iprecision = precision_score(pred, test_labels)
irecall = recall_score(pred, test_labels)

sk_cls = SVC(max_iter=150)
sk_cls.fit(train_images, train_labels)
sk_pred = sk_cls.predict(test_images)
lf1 = f1_score(sk_pred, test_labels)
lprecision = precision_score(sk_pred, test_labels)
lrecall = recall_score(sk_pred, test_labels)
print(f'F1 (sklearn): {lf1:.4f}')
print(f"Precision (sklearn): {lprecision:.4f}")
print(f"Recall (sklearn): {lrecall:.4f}")
print(f"F1-offset: {abs(lf1-if1):.5f}")
print(f"Precision-offset: {abs(lprecision-iprecision):.5f}")
print(f"Recall-offset: {abs(lrecall-irecall):.5f}")
